...
class WiFiClientSecure : public WiFiClient {
  ...
  public:
	...
	int connect(IPAddress ip, uint16_t port, uint16_t localPort) override { return _ctx->connect(ip, port, localPort); }
	...
	int connect(IPAddress ip, uint16_t port, const char* name) { return _ctx->connect(ip, port, name); }
	int connect(IPAddress ip, uint16_t port, uint16_t localPort, const char* name) { return _ctx->connect(ip, port, localPort, name); }
