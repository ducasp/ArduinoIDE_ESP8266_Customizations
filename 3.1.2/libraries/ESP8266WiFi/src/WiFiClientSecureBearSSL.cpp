...
int WiFiClientSecureCtx::connect(IPAddress ip, uint16_t port, uint16_t localPort) {
...
int WiFiClientSecureCtx::connect(IPAddress ip, uint16_t port, uint16_t localPort, const char* name) {
  if (!WiFiClient::connect(ip, port,localPort)) {
    return 0;
  }
...
int WiFiClientSecureCtx::connect(IPAddress ip, uint16_t port, const char* name) {
  if (!WiFiClient::connect(ip, port)) {
    return 0;
  }
  return _connectSSL(name);
